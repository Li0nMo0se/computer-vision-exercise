...
class Tracker:
    ...
    def getCorrespondencePoints(self, keypoints, matches, matchingRatio = 0.7):
        ...
        # Lowe's ratio m.distance / n.distance < alpha
        # k was 2 thus only two matches (m, n) per matche
        for m, n in matches:
            # if m.distance / n.distance >= 0.9 the distances are too similar
    ...
    def computeHomography(self, targetPoints, observedPoints, minMatches = 12, weight = 0.4):
        ...
        if len(observedPoints) > minMatches:
            ...
            homography, _ = cv.findHomography(targetPoints, observedPoints, method=cv.RANSAC)

            if len(self.homography) != 0 and len(homography) != 0: # use if not self.initialized
                homography = cv.accumulateWeighted(self.homography, homography, weight)
    ...
    def drawObjectContours(self, image):
        if self.homography is not None and len(self.homography) > 0:
