...
class Registration:
    def __init__(self, source, target, voxelSize = 0.05):
        ...
        self.result = o3d.registration.RegistrationResult()
        ...
        self.sourceFpfh = o3d.registration.compute_fpfh_feature(self.sourceDown, o3d.geometry.KDTreeSearchParamHybrid(radius=self.voxelSize * 5, max_nn=100))
        self.targetFpfh = o3d.registration.compute_fpfh_feature(self.targetDown, o3d.geometry.KDTreeSearchParamHybrid(radius=self.voxelSize * 5, max_nn=100))
    ...
    def processGlobal(self):
        ...
        result = o3d.registration.registration_ransac_based_on_feature_matching(self.sourceDown,
                                                                                self.targetDown,
                                                                                self.sourceFpfh,
                                                                                self.targetFpfh,
                                                                                maxCorrespondanceDistance)



        self.result = o3d.registration.evaluate_registration(self.source, self.target, 0.02, result.transformation)
        print(self.result)
        return self.result
    ...
    def processICP(self, pointToPlane = False):
        ...
        if not pointToPlane:
            ...
            result = o3d.registration.registration_icp(self.source, self.target, distanceThreshold, self.result.transformation,
